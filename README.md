...
- **매일 자정(한국시간)** 에도 한 번 확인합니다(`triggers.crons` = `0 15 * * *`, UTC 기준) — 변동이 있으면 뜨고, 없으면 오래된 것만 정리합니다.
- **보관 기간 1주일.** 지난 것은 지워지되 **가장 최근 5개는 나이와 상관없이 남습니다**(백업이 0개가 되는 상황 방지). 더 오래 두려면 파일로 내려받아 두세요.
